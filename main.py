...
class ForexTradingStrategy:
    ...
    def fetch_data(self):
        try:
            url = f'https://www.alphavantage.co/query?function=FX_INTRADAY&from_symbol={self.symbol[:3]}&to_symbol={self.symbol[3:]}&interval=15min&outputsize=full&apikey={self.api_key}&datatype=csv'
...
symbol = st.sidebar.text_input('Forex Symbol (e.g., EURUSD)', value='EURUSD')
...
if st.sidebar.button('Run Backtest'):
    if not api_key:
        ...
        with st.spinner('Running backtest...'):
            try:
                strategy = ForexTradingStrategy(symbol, start_date, end_date, rsi_period, rsi_overbought, rsi_oversold, api_key)
